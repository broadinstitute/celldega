...
spec = importlib.util.spec_from_file_location("celldega.nbhd.neighborhoods", NBHD_ROOT / "neighborhoods.py")
...
def synthetic_data():
    ...
    gdf_nbhd = gpd.GeoDataFrame(
        {"name": ["A", "B"], "geometry": nbhd_polys}, crs="EPSG:4326"
    )

    coords = [(1, 1)] * 6 + [(11, 1)] * 4
    clusters = ["X", "Y", "X", "X", "Y", "Y", "X", "X", "Y", "Y"]
    gdf_cells = gpd.GeoDataFrame(
        {"cluster": clusters, "geometry": [Point(xy) for xy in coords]},
        crs="EPSG:4326"
    )
