...
def batch_transform_geometries(geometries, transformation_matrix, scale):
    ...
    for geom in geometries:
        if isinstance(geom, Point):
            ...
            polygons = [
                Polygon(
                    numpy_affine_transform(np.array(p.exterior.coords), affine_matrix)
                    / scale,
                    [
                        numpy_affine_transform(np.array(interior.coords), affine_matrix)
                        / scale
...
def get_cell_polygons(
    technology, path_cell_boundaries, path_output=None, path_meta_cell_micron=None
):
...
def make_cell_boundary_tiles(
    ...
    gdf_cells = get_cell_polygons(
        technology, path_cell_boundaries, path_output, path_meta_cell_micron
    )

    # Transform geometries
    gdf_cells["GEOMETRY"] = batch_transform_geometries(
        gdf_cells["geometry"], transformation_matrix, image_scale
    )
