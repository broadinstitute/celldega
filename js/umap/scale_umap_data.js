...
export const scale_umap_data = (viz_state, cell_scatter_data_objects) => {
    ...
    const {x_min} = viz_state.spatial // d3.min(cell_scatter_data_objects.map(d => d.position[0]))
    const {x_max} = viz_state.spatial // d3.max(cell_scatter_data_objects.map(d => d.position[0]))
    const {y_min} = viz_state.spatial // d3.min(cell_scatter_data_objects.map(d => d.position[1]))
    const {y_max} = viz_state.spatial // d3.max(cell_scatter_data_objects.map(d => d.position[1]))
