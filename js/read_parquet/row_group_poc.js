...
export async function readRowGroups(url, rowGroups = null) {
  ...
  console.log(`[row_group_poc] Parquet file size: ${(data.length / 1024).toFixed(2)} KB`);
  ...
  console.log(`[row_group_poc] Read ${table.numRows} rows, ${table.numCols} columns`);
  console.log(`[row_group_poc] Schema:`, table.schema.fields.map(f => f.name));
...
export async function getParquetMetadata(url) {
  ...
  const numBatches = wasmTable.numBatches;
  const schema = wasmTable.schema;
  ...
  return {
    numBatches,
    columns: arrowSchema.fields.map(f => ({ name: f.name, type: f.type.toString() })),
  };
}
...
export async function testRowGroupReading(url) {
  ...
  try {
    ...
    if (rgIdCol) {
      ...
      console.log(`  Row group IDs present: [${[...uniqueGroupIds].sort().join(', ')}]`);
    }
