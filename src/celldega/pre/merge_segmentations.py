...
def merge_segmentation(
    ...
    transformation_matrix = pd.read_csv(
        f"{custom_data_path}/micron_to_image_transform.csv", sep=" ", header=None
    ).values[:3, :3]
    ...
    if os.path.exists(f"{custom_data_path}/cutout_region_alpha_shape.parquet"):
        ...
        largest_cutout_region_alpha_shape = largest_cutout_region_alpha_shape_gdf.loc[
            0
        ]["geometry"]
        ...
        filtered_cells_default_within_cutout_region = cells_default[
            cells_default.index.isin(cells_default_within_clusters.index.to_list())
        ]

        filtered_cells_default_within_cutout_region["centroid"] = (
            filtered_cells_default_within_cutout_region["geometry"].centroid
        )
        filtered_cell_centroids_default_within_cutout_region = np.array(
            list(
                zip(
                    filtered_cells_default_within_cutout_region["centroid"].x,
                    filtered_cells_default_within_cutout_region["centroid"].y,
                )
            )
        )

        cutout_region_alpha_shape = alpha_shape(
            filtered_cell_centroids_default_within_cutout_region,
        ...
        if len(cutout_region_alpha_shape.geoms) == 1:
            ...
            largest_cutout_region_alpha_shape_temp = max(
                cutout_region_alpha_shape.geoms, key=lambda p: p.area
            )
            largest_cutout_region_alpha_shape = (
                largest_cutout_region_alpha_shape_temp.buffer(
    ...
    indices_cells_default_within_cutout_region = (
        cells_default_within_cutout_region.index
    )
    ...
    cells_before_harmonization["centroid"] = cells_before_harmonization[
        "geometry"
    ].centroid
    ...
    gpd.GeoDataFrame(geometry=[largest_cutout_region_alpha_shape]).to_parquet(
        f"{output_path}/cutout_region_alpha_shape.parquet", index=False
    )
    ...
    indices_intersecting_cells_before_harmonization = (
        intersecting_cells_before_harmonization.index.tolist()
    )
    intersecting_cell_pairs_before_harmonization = (
        intersecting_cells_before_harmonization.sindex.query(
            intersecting_cells_before_harmonization.geometry, predicate="intersects"
        )
    )

    intersecting_cell_pairs_before_harmonization_DF = pd.DataFrame(
        intersecting_cell_pairs_before_harmonization
    ).T
    intersecting_cell_pairs_before_harmonization_DF.columns = ["cell_id_1", "cell_id_2"]

    intersecting_cell_pairs_before_harmonization_DF["cell_id_1"] = (
        intersecting_cells_before_harmonization.index[
            intersecting_cell_pairs_before_harmonization_DF["cell_id_1"]
        ].values
    )
    intersecting_cell_pairs_before_harmonization_DF["cell_id_2"] = (
        intersecting_cells_before_harmonization.index[
            intersecting_cell_pairs_before_harmonization_DF["cell_id_2"]
        ].values
    )
    intersecting_cell_pairs_before_harmonization_DF["cell_id_1_technology"] = (
        intersecting_cells_before_harmonization.technology[
            intersecting_cell_pairs_before_harmonization_DF["cell_id_1"]
        ].values
    )
    intersecting_cell_pairs_before_harmonization_DF["cell_id_2_technology"] = (
        intersecting_cells_before_harmonization.technology[
            intersecting_cell_pairs_before_harmonization_DF["cell_id_2"]
        ].values
    )

    intersecting_cell_pairs_before_harmonization_DF = (
        intersecting_cell_pairs_before_harmonization_DF[
            intersecting_cell_pairs_before_harmonization_DF["cell_id_1"]
            != intersecting_cell_pairs_before_harmonization_DF["cell_id_2"]
        ]
    ...
    intersecting_cell_pairs_before_harmonization_DF["sorted_cell_id_pair"] = (
        intersecting_cell_pairs_before_harmonization_DF.apply(
            lambda row: tuple(sorted([row["cell_id_1"], row["cell_id_2"]])), axis=1
        )
    )

    intersecting_cell_pairs_before_harmonization_DF = (
        intersecting_cell_pairs_before_harmonization_DF.drop_duplicates(
            subset="sorted_cell_id_pair"
        )
    )

    intersecting_cell_pairs_before_harmonization_DF = (
        intersecting_cell_pairs_before_harmonization_DF.drop(
            columns=["sorted_cell_id_pair"]
        )
    )
    ...
    list_no_conflict_cell_ids = sorted(
        list(
            set(indices_intersecting_cells_before_harmonization).difference(
                set(list_conflict_cell_ids)
            )
        )
    )
    ...
    for inst_row in intersecting_cell_pairs_before_harmonization_DF.index.tolist():
        ...
        id_1 = intersecting_cell_pairs_before_harmonization_DF.loc[
            inst_row, "cell_id_1"
        ]
        id_2 = intersecting_cell_pairs_before_harmonization_DF.loc[
            inst_row, "cell_id_2"
        ]
        ...
        if area_1 > 0 and area_2 > 0:
            ...
            intersecting_cell_pairs_before_harmonization_DF.loc[inst_row, "ioa_1"] = (
                ioa_1
            )
            intersecting_cell_pairs_before_harmonization_DF.loc[inst_row, "ioa_2"] = (
                ioa_2
            )
            intersecting_cell_pairs_before_harmonization_DF.loc[
                inst_row, "ioa_small"
            ] = ioa_small
    ...
    intersecting_cell_pairs_before_harmonization_DF.sort_values(
        by="ioa_small", ascending=False, inplace=True
    )
    ...
    for inst_row in intersecting_cell_pairs_before_harmonization_DF.index.tolist():
        ...
        tech_1 = intersecting_cell_pairs_before_harmonization_DF.loc[
            inst_row, "cell_id_1_technology"
        ]
        tech_2 = intersecting_cell_pairs_before_harmonization_DF.loc[
            inst_row, "cell_id_2_technology"
        ]
        ...
        if inst_ioa_small < ioa_small_thresh:
            ...
            gdf_nc = add_or_merge_into_gdf_nc(
                gdf_nc=gdf_nc,
                default_clustering=default_clustering,
                clusters_within_cutout_region=clusters_within_cutout_region,
                poly=poly_1,
                ioa_thresh=ioa_small_thresh,
                ID=id_1,
                tech=tech_1,
            )
            gdf_nc = add_or_merge_into_gdf_nc(
    ...
    remaining_cells = cells_before_harmonization[
        ~cells_before_harmonization["geometry"].intersects(intersecting_boundary)
    ]

    merged_cells = gpd.GeoDataFrame(
        pd.concat([remaining_cells, gdf_nc], ignore_index=False)
    )
    ...
    segmentation_parameters = {
        "technology": "custom",
        "segmentation_approach": custom_segmentation_parameters["segmentation_approach"]
        + "_"
