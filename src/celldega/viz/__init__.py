...
def landscape_clustergram(landscape, mat, width="600px", height="700px"):
    ...
    """
    # Ensure widgets have an active comm before linking. `AnyWidget` instances
    # do not instantiate a dummy Comm until ``open()`` is called which can
    # cause ``jslink`` to fail with ``AttributeError`` when accessing
    # ``model_id``.
    if getattr(mat, "comm", None) is None:
        mat.open()
    if getattr(landscape, "comm", None) is None:
        landscape.open()

    # Use `jslink` to directly link `click_info` from `mat` to
    # `update_trigger` in `landscape`
