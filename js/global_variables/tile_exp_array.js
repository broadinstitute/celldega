...
export const update_tile_exp_array = async (viz_state, inst_gene) => {
  const base_url = viz_state.global_base_url;
  ...
  const new_exp_array = new Array(viz_state.cats.tile_names_array.length).fill(
    0
  );
  ...
  tile_names.forEach((name, i) => {
    if (viz_state.cats.tile_name_to_index_map.has(name)) {
      ...
      const max_exp = Math.log(
        Number(viz_state.genes.meta_gene[inst_gene].max)
      );
